...
// Getrennte Google-Verbindungen: Kalender und Tasks können mit
// unterschiedlichen Google-Konten verbunden werden.
const googleConn = {
  calendar: { accessToken: null, tokenExpiry: 0, tokenClient: null, scope: SCOPE_CALENDAR },
  tasks: { accessToken: null, tokenExpiry: 0, tokenClient: null, scope: SCOPE_TASKS },
};
let reviewItems = [];
...
async function toggleTodo(id) {
  ...
  if (todo.googleId) {
    const token = await getValidToken("tasks");
    if (token) {
      try {
...
function ensureTokenClient(kind) {
  if (!window.google || !google.accounts || !google.accounts.oauth2) return null;
  if (!state.clientId) return null;
  const conn = googleConn[kind];
  if (!conn.tokenClient) {
    conn.tokenClient = google.accounts.oauth2.initTokenClient({
      client_id: state.clientId,
      scope: conn.scope,
      callback: () => {}, // wird pro Aufruf überschrieben
    });
  }
  return conn.tokenClient;
}

function requestToken(kind, interactive) {
  return new Promise((resolve) => {
    const client = ensureTokenClient(kind);
    const conn = googleConn[kind];
    if (!client) { resolve(null); return; }
    client.callback = (resp) => {
      if (resp && resp.access_token) {
        conn.accessToken = resp.access_token;
        conn.tokenExpiry = Date.now() + (resp.expires_in || 3500) * 1000;
        updateGoogleStatusUI(kind, true);
    ...
    try {
      // "select_account" zeigt immer die Kontoauswahl, damit Kalender- und
      // Tasks-Konto bewusst unterschiedlich gewählt werden können.
...
async function getValidToken(kind) {
  const conn = googleConn[kind];
  if (conn.accessToken && Date.now() < conn.tokenExpiry - 30000) return conn.accessToken;
  if (!navigator.onLine) return null;
  return requestToken(kind, false);
}

function updateGoogleStatusUI(kind, connected) {
  const label = kind === "calendar" ? "Kalender" : "Tasks";
  const dot = document.getElementById(`${kind}StatusDot`);
  const text = document.getElementById(`${kind}StatusText`);
  const connectBtn = document.getElementById(`connect${kind === "calendar" ? "Calendar" : "Tasks"}Btn`);
  const disconnectBtn = document.getElementById(`disconnect${kind === "calendar" ? "Calendar" : "Tasks"}Btn`);
  if (connected) {
    dot.className = "status-dot online";
    text.textContent = `Verbunden – ${label === "Kalender" ? "Termine" : "To-dos"} werden synchronisiert`;
    connectBtn.textContent = "Konto wechseln";
    disconnectBtn.classList.remove("hidden");
  } else {
    dot.className = "status-dot offline";
    text.textContent = state.clientId ? "Nicht verbunden" : "Erst Client-ID eintragen";
    connectBtn.textContent = `${label}-Konto verbinden`;
    disconnectBtn.classList.add("hidden");
  }
}

function setupGoogleConnectionButtons(kind, btnIdConnect, btnIdDisconnect) {
  document.getElementById(btnIdConnect).addEventListener("click", async () => {
    if (!state.clientId) {
    ...
    const token = await requestToken(kind, true);
    if (token) {
      showToast(`${kind === "calendar" ? "Kalender" : "Tasks"}-Konto verbunden.`);
      trySync();
    } else {
  ...
  document.getElementById(btnIdDisconnect).addEventListener("click", () => {
    const conn = googleConn[kind];
    if (conn.accessToken && window.google && google.accounts) {
      try { google.accounts.oauth2.revoke(conn.accessToken, () => {}); } catch (e) {}
    }
    conn.accessToken = null;
    conn.tokenExpiry = 0;
    updateGoogleStatusUI(kind, false);
    showToast("Verbindung getrennt.");
  });
}

setupGoogleConnectionButtons("calendar", "connectCalendarBtn", "disconnectCalendarBtn");
setupGoogleConnectionButtons("tasks", "connectTasksBtn", "disconnectTasksBtn");

document.getElementById("saveClientIdBtn").addEventListener("click", () => {
  const val = document.getElementById("clientIdInput").value.trim();
  state.clientId = val;
  saveState();
  googleConn.calendar.tokenClient = null;
  googleConn.calendar.accessToken = null;
  googleConn.calendar.tokenExpiry = 0;
  googleConn.tasks.tokenClient = null;
  googleConn.tasks.accessToken = null;
  googleConn.tasks.tokenExpiry = 0;
  updateGoogleStatusUI("calendar", false);
  updateGoogleStatusUI("tasks", false);
...
async function trySync() {
  ...
  if (pendingTodos.length > 0) {
    const tasksToken = await getValidToken("tasks");
    if (tasksToken) {
  ...
  if (pendingEvents.length > 0) {
    const calendarToken = await getValidToken("calendar");
    if (calendarToken) {
...
document.getElementById("openSettings").addEventListener("click", () => {
  ...
  updateGoogleStatusUI("calendar", !!googleConn.calendar.accessToken);
  updateGoogleStatusUI("tasks", !!googleConn.tasks.accessToken);
  renderRecipients();
